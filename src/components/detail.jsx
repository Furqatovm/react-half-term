  ...
  <div className="grid items-start grid-cols-1 lg:grid-cols-3">
    <div className="col-span-2 grid grid-cols-2 lg:sticky top-0 gap-0.5">
      <div className="columns-2 gap-0.5 space-y-0.5">
        {product?.images?.map((nat, index) =>{
            return <div key={index} className="overflow-hidden">
            <img src={nat} alt="Product" className="w-full aspect-[253/337] object-cover object-top shadow-md hover:scale-[1.05] transition-all duration-300" />
          </div>
        })}
      </div>
      <div className="overflow-hidden">
        <img src={product?.thumbnail} alt="Product" className="w-full aspect-[3/4] object-cover object-top shadow-md hover:scale-[1.05] transition-all duration-300" />
      </div>
    </div>
    <div className="py-6 px-8 max-lg:max-w-2xl">
      <div>
        <h2 className="text-xl font-semibold text-slate-900">{product?.title}</h2>
        <p className="text-sm text-slate-500 mt-2">{product?.brand}</p>
      </div>
      <div className="flex items-center space-x-1 mt-6">
       {Array.from({length: product?.rating}, (_, i) =>{
        return <svg  key={i} className="w-4 h-4 fill-slate-800" viewBox="0 0 14 13" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="mt-8">
        <div className="flex items-center flex-wrap gap-4">
          <p className="text-slate-900 text-4xl font-semibold">${product?.price}</p>
      ...
      <div className="mt-8 space-y-4">
        <button
         type="button" className="w-full px-4 py-2.5 cursor-pointer border border-slate-800 bg-transparent hover:bg-slate-50 text-slate-900 text-sm font-medium rounded-md">Add to cart</button>
        <Link to={"/shop"} >
      ...
      <div className="mt-8">
        <div>
          <h3 className="text-xl font-semibold text-slate-900">{product?.title} Description</h3>
